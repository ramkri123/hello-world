...
class ConsortiumComparisonService:
    def __init__(self):
        ...
        # Initialize privacy-preserving NLP processor
        try:
            from privacy_preserving_nlp import PrivacyPreservingNLP
            self.nlp_processor = PrivacyPreservingNLP()
            print("✅ Privacy-preserving NLP processor initialized")
        except ImportError:
            self.nlp_processor = None
            print("⚠️ Privacy-preserving NLP not available - using feature arrays only")
    ...
    def process_raw_transaction(self, transaction_data: Dict, email_content: str = "", 
                              sender_data: Dict = None, receiver_data: Dict = None) -> Dict[str, Any]:
        """Process raw transaction with privacy-preserving NLP feature extraction"""
        if self.nlp_processor is None:
            raise ValueError("Privacy-preserving NLP not available. Use generate_comparison_score with feature arrays.")
        
        print("🔒 PRIVACY-PRESERVING TRANSACTION ANALYSIS")
        print("="*50)
        print(f"📧 Email Content: {len(email_content)} characters")
        print(f"💰 Transaction Amount: ${transaction_data.get('amount', 0):,.2f}")
        print(f"🏦 Processing with consortium NLP...")
        
        # Extract anonymous features using NLP
        anonymous_features = self.nlp_processor.convert_to_anonymous_features(
            transaction_data, email_content, sender_data, receiver_data
        )
        
        print(f"✅ Generated {len(anonymous_features)} anonymous features")
        print(f"🔒 EMAIL CONTENT DELETED (privacy preserved)")
        print()
        
        # Process using standard comparison scoring
        return self.generate_comparison_score(anonymous_features)
